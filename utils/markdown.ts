...
export const createPptxFile = async (data: PresentationData, topic: string): Promise<void> => {
    try {
        ...
        // Define Master for the Title Slide with placeholders for stability
        pptx.defineSlideMaster({
            title: 'AIKON_TITLE_MASTER',
            background: { color: AIKON_BG_COLOR },
            objects: [
                { 'placeholder': {
                    options: { name: 'title', type: 'title', x: 0.5, y: 2, w: '90%', h: 1, fontFace: 'Inter', fontSize: 44, bold: true, color: AIKON_PRIMARY_COLOR, align: 'center' },
                    text: '(Title)'
                }},
                { 'placeholder': {
                    options: { name: 'subtitle', type: 'body', x: 0.5, y: 3.2, w: '90%', h: 0.5, fontFace: 'Inter', fontSize: 18, color: AIKON_TEXT_COLOR, align: 'center' },
                    text: '(Subtitle)'
                }}
            ],
        });
        ...
        pptx.defineSlideMaster({
            ...
            objects: [
                ...
                { 'placeholder': {
                    options: { name: 'title', type: 'title', x: 0.5, y: 0.5, w: '90%', h: 0.75, fontFace: 'Inter', fontSize: 28, bold: true, color: AIKON_PRIMARY_COLOR },
                    text: '(Title)'
                }},
                { 'placeholder': {
        ...
        // Title Slide using its own master and populating placeholders
        const titleSlide = pptx.addSlide({ masterName: 'AIKON_TITLE_MASTER' });
        titleSlide.addText(topic, { placeholder: 'title' });
        titleSlide.addText(`A presentation by Aikon Studios`, { placeholder: 'subtitle' });

        // Content Slides using the content master
        for (const slideData of data.slides) {
            const slide = pptx.addSlide({ masterName: 'AIKON_CONTENT_MASTER' });
            // Populate the placeholders
            slide.addText(slideData.title, { placeholder: 'title' });
            slide.addText(slideData.content, { placeholder: 'body' }); // Pass array directly for bullet points
